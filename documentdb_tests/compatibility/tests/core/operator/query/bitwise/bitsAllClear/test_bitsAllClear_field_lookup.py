"""
Tests for $bitsAllClear field lookup patterns, cross-type combinations, and algebraic properties.
...
FIELD_LOOKUP_TESTS: list[QueryTestCase] = [
    QueryTestCase(
        ...
        expected=[{"_id": 1, "a": 20}],
        msg="Simple field path matches when bits 0,1 are clear in 20 (10100)",
    ),
    QueryTestCase(
        id="nested_field_dot_notation",
        filter={"a.b": {"$bitsAllClear": [0, 1]}},
        doc=[{"_id": 1, "a": {"b": 20}}, {"_id": 2, "a": {"b": 3}}],
    ...
    QueryTestCase(
        ...
        filter={"a": {"$bitsAllClear": [0, 1]}},
        doc=[{"_id": 1, "a": [20, 54]}, {"_id": 2, "a": [3, 7]}],
        expected=[{"_id": 1, "a": [20, 54]}],
        msg="Array field matches if any element satisfies",
    ),
    QueryTestCase(
        id="array_index_path",
        filter={"a.0": {"$bitsAllClear": [0, 1]}},
        doc=[{"_id": 1, "a": [20, 54]}, {"_id": 2, "a": [3, 20]}],
        expected=[{"_id": 1, "a": [20, 54]}],
        msg="Array index path checks element at index 0",
    ),
    QueryTestCase(
        id="deep_nested_path_through_array_of_objects",
        filter={"a.b.c.d": {"$bitsAllClear": [0, 1]}},
        doc=[
            {"_id": 1, "a": {"b": [{"c": {"d": 20}}, {"c": {"d": 3}}]}},
            {"_id": 2, "a": {"b": [{"c": {"d": 1}}, {"c": {"d": 7}}]}},
        ],
        expected=[{"_id": 1, "a": {"b": [{"c": {"d": 20}}, {"c": {"d": 3}}]}}],
...
CROSS_TYPE_TESTS: list[QueryTestCase] = [
    QueryTestCase(
        ...
        filter={"a": {"$bitsAllClear": Binary(b"\x01")}},
    ...
    QueryTestCase(
        ...
        filter={"a": {"$bitsAllClear": Binary(b"\x01\x00\x00")}},
        doc=[{"_id": 1, "a": Binary(b"\x06", 128)}, {"_id": 2, "a": Binary(b"\x01", 128)}],
        expected=[{"_id": 1, "a": Binary(b"\x06", 128)}],
...
ALGEBRAIC_TESTS: list[QueryTestCase] = [
    ...
    QueryTestCase(
        ...
        filter={"a": {"$bitsAllClear": Binary(b"\x23")}},
        doc=[{"_id": 1, "a": 0}, {"_id": 2, "a": 35}, {"_id": 3, "a": 220}],
        expected=[{"_id": 1, "a": 0}, {"_id": 3, "a": 220}],
...
def test_bitsAllClear_field_lookup(collection, test):
    """Test $bitsAllClear field lookup, cross-type combinations, and algebraic properties."""
